...
`;function b(u){const r=v(),t=[];if(u.color!==void 0){const n=x(r,u.color,P);t.push(`color = ${n};`)}const i=Object.keys(r.variables);if(i.length>1&&!u.variables)throw new Error(`Missing variables in style (expected ${r.variables})`);const o={};for(const n of i){if(!(n in u.variables))throw new Error(`Missing '${n}' in style variables`);const _=g(n);o[_]=function(){let p=u.variables[n];return typeof p=="string"&&(p=C(p)),p!==void 0?p:-9999999}}const s=Object.keys(o).map(function(n){return`uniform float ${n};`}),l=Object.keys(r.functions).map(function(n){return r.functions[n]}),h=`
    #ifdef GL_FRAGMENT_PRECISION_HIGH
    precision highp float;
    #else
    precision mediump float;
...
`)}
  ...
  `;return{tileVertexShader:y,tileFragmentShader:D,particleColorVertexShader:F,particleColorFragmentShader:h,particlePositionVertexShader:O,particlePositionFragmentShader:X,textureVertexShader:O,textureFragmentShader:U}}const I=[];class L extends N{constructor(r){const t=Object.assign({},r);if(delete t.maxSpeed,delete t.speedFactor,delete t.particles,super(t),this.style_=r.style||{},!(r.maxSpeed>0))throw new Error("maxSpeed is required");this.maxSpeed_=r.maxSpeed,this.speedFactor_=r.speedFactor,this.particles_=r.particles,this.styleVariables_=this.style_.variables||{},this.addChangeListener($.SOURCE,this.handleSourceUpdate_)}handleSourceUpdate_(){this.hasRenderer()&&this.getRenderer().clearCache()}updateStyleVariables(r){Object.assign(this.styleVariables_,r),this.changed()}getSources(r,t){const i=this.getSource();return I[0]=i,I}createRenderer(){const r=b(this.style_);return new A(this,{...r,cacheSize:this.getCacheSize(),maxSpeed:this.maxSpeed_,speedFactor:this.speedFactor_,particles:this.particles_})}}L.prototype.dispose;export{L as default};
//# sourceMappingURL=Flow-B2xuAFhC.js.map
